...
def info_extract(df):
    ...
    if not confirm == "A":
        while True:
            col = inp("Which column do you want to add or remove?", default=skip_string).strip()
            if col == skip_string:
                break
                ...
                err(f"We cannot find the column \"{col}\" in the imported DataFrame. "
                    f"Please check your spelling and try again (case-sensitive).")
